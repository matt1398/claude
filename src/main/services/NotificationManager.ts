 ...
 *
 * Responsibilities:
 * - Store error history at ~/.claude/viz-notifications.json (max 100 entries)
...
/**
 * Detected error from JSONL parsing.
 * Imported conceptually from ErrorDetector - defined here for standalone usage.
 */
export interface DetectedError {
  ...
  timestamp: number;
  /** Session UUID where error occurred */
  sessionId: string;
  /** Encoded project ID */
  projectId: string;
  ...
  /** Error message */
  message: string;
  /** Line number in JSONL file for deep linking */
  lineNumber?: number;
  /** Additional context */
  context: {
...
const NOTIFICATIONS_PATH = path.join(os.homedir(), '.claude', 'viz-notifications.json');
...
export class NotificationManager extends EventEmitter {
  ...
  initialize(): void {
    ...
    console.log(`NotificationManager: Initialized with ${this.notifications.length} notifications`);
  }
    ...
    try {
      if (fs.existsSync(NOTIFICATIONS_PATH)) {
        ...
        if (Array.isArray(parsed)) {
          ...
          console.warn('NotificationManager: Invalid notifications file format, starting fresh');
          this.notifications = [];
        }
      }
    } catch (error) {
  ...
  private saveNotifications(): void {
    try {
      ...
      console.error('NotificationManager: Error saving notifications:', error);
    }
  }
  ...
  private pruneNotifications(): void {
    if (this.notifications.length > MAX_NOTIFICATIONS) {
      ...
      console.log(`NotificationManager: Pruned ${removed} old notifications`);
    }
  }
  ...
  private matchesIgnoredRegex(error: DetectedError): boolean {
    ...
    for (const pattern of patterns) {
      try {
        ...
        console.warn(`NotificationManager: Invalid regex pattern: ${pattern}`);
      }
    }
  ...
  /**
   * Checks if the error is from an ignored project.
   */
  private isFromIgnoredProject(error: DetectedError): boolean {
    const config = this.configManager.getConfig();
    const ignoredProjects = config.notifications.ignoredProjects;

    if (!ignoredProjects || ignoredProjects.length === 0) {
      return false;
    }

    return ignoredProjects.includes(error.projectId);
  }

  /**
   * Determines if an error should generate a notification.
   */
  private shouldNotify(error: DetectedError): boolean {
    // Check if notifications are enabled
    if (!this.areNotificationsEnabled()) {
      return false;
    }

    // Check if error is from an ignored project
    if (this.isFromIgnoredProject(error)) {
      return false;
    }
  ...
  private showNativeNotification(error: DetectedError): void {
    ...
    if (!Notification.isSupported()) {
      console.warn('NotificationManager: Native notifications not supported');
      return;
    }
   ...
   */
  addError(error: DetectedError): StoredNotification | null {
